...
- Дополнительные расходы: доставка (7%), сертификация (3%), накладные (5%) = +15% к рыночной стоимости
- Итоговые затраты = рыночная_стоимость + дополнительные_расходы
- Чистую маржу: тендерная_цена - итоговые_затраты
- Процент маржинальности: (маржа / тендерная_цена) × 100%

## 2. АНАЛИЗ ЗАКАЗЧИКА
ПРОАНАЛИЗИРУЙ:
- Полное название организации и ИНН
- Тип заказчика (государственная/муниципальная/коммерческая)
- Особенности работы с данным типом заказчика
- Оценка надежности (госструктуры = низкий риск неплатежа)
...
ОЦЕНИ:
- Ограничения для участников (МСП, лицензии, опыт)
...
Дополнительные расходы: [сумма] рублей (доставка, сертификация, накладные)
Итоговые затраты: [сумма] рублей
...
Дополнительные расходы (15%): [сумма] рублей
Общие затраты: [сумма] рублей

=== АНАЛИЗ ЗАКАЗЧИКА ===

Организация: [полное название]
ИНН: [ИНН]
Тип: [государственная/муниципальная/коммерческая организация]
Надежность: [оценка риска неплатежа]
Особенности: [специфика работы с данным заказчиком]
...
Ограничения участников: [МСП, лицензии, опыт]
Ожидаемое количество участников: [оценка]
Барьеры входа: [низкие/средние/высокие]
Географический фактор: [влияние локации поставки]
...
Срок подачи заявок: [дата]
Место поставки: [адрес]
Условия оплаты: [сроки и условия]
...
4. ВСЕГДА анализируй заказчика по предоставленным данным
5. Ответ должен быть готов для отправки в Telegram бот без изменений
6. Все расчеты округляй до целых рублей
7. Используй только простой текст и символы === для разделения разделов
8. Никогда не отвечай в табличном виде.
`,
};
